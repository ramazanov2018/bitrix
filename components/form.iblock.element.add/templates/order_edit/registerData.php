...
if(isset($regDataProps)){
    $reservedDateTime = [];
        ...
        $obj = \Bitrix\Iblock\Elements\ElementMterecordTable::getList([
            'select' => [
                'ID',
                'REG_DATE' => 'REGISTRY_DATE.VALUE',
                'REG_TIME' => 'REGISTRY_TIME.VALUE'
            ],
            'order' => ['sort' => 'ASC'],
            'filter' =>  [
                    '<=REG_DATE' =>  BXDateTime::createFromTimestamp(strtotime('Monday next week'))->add("6 day")->format('Y-m-d'),
                    '>=REG_DATE' =>  BXDateTime::createFromTimestamp(strtotime('Monday this week'))->format('Y-m-d')
            ],
        ]);

        while ($row = $obj->fetch()) {
            $row['REG_DATE'] .= " ";
            $row['REG_DATE'] .= " " . strlen($row["REG_TIME"])>0?$row["REG_TIME"]:"00:00";
            $row['REG_DATE'] .= ":00";
            $dt = new BXDateTime($row['REG_DATE'], "Y-m-d H:i:s");
            $reservedDateTime[$dt->getTimestamp()][] = $row["ID"];
        }
        $obj = \Bitrix\Iblock\Elements\ElementRegisterappTable::getList([
            'select' => [
                'ID',
                'REG_DATE' => 'REGISTRY_DATE.VALUE',
                'REG_TIME' => 'REGISTRY_TIME.VALUE'
        ...
        $obj = \Bitrix\Iblock\Elements\ElementRegistertcpTable::getList([
            'select' => [
                'ID',
                'REG_DATE' => 'REGISTRY_DATE.VALUE',
                'REG_TIME' => 'REGISTRY_TIME.VALUE'
    ...
    $mtkRegisterData = RcDimosHelpers::getHlBlockData(HLB_MTK,
        ["ID", "UF_NAME", "UF_XML_ID", "UF_SLOT_DURATION", "UF_PERIOD_START", "UF_PERIOD_END", "UF_BREAK_START", "UF_BREAK_END", "UF_WEEKENDS", "UF_HOLIDAYS", "UF_SHORT_DAYS", "UF_SHORT_PERIOD_END"],
        [],
        ["UF_XML_ID" => HBL_ELEMENT_SCHEDULE_SHEDULE_TOTAL]
    );
    ...
    if(is_array($mtkRegisterData)) {
        ...
        $ShortPeriodEnd = (!empty($ScheduleData['UF_SHORT_PERIOD_END'])) ? $ScheduleData['UF_SHORT_PERIOD_END'] : HLB_TOPICS_PROPERTY_PERIOD_END;
		...
		//короткие дни
        if(!empty($ScheduleData['UF_SHORT_DAYS'])){
            foreach ($ScheduleData['UF_SHORT_DAYS'] as $topicWeekend){
                $rsPropWeekend = CUserFieldEnum::GetList([], ["ID" => $topicWeekend]);
                if($arPropWeekend = $rsPropWeekend->GetNext()){
                    $ShortDays[$arPropWeekend['XML_ID']] = explode("_", $arPropWeekend['XML_ID'])[1];
                }
            }
        }
        //Праздничные дни
                                ...
                                <?if(!in_array($date['WEEK_DAY'], $arWeekends) && !in_array($date['TIME_FORMAT'], $arHolidays)):?>
                                    <div class="talon">
                                        <div class="disabled">
                                            <?php
											$end = $periodEnd;
                                			if(in_array($date['WEEK_DAY'], $ShortDays))
                                    			$end = $ShortPeriodEnd;
                                            ...
                                            if(key_exists($date['TIME_FORMAT'], $arHolidaySchedule)){
                                                ...
                                                $arMinutes = getMinutes($periodStart, $end, $breakStart, $breakEnd, $duration, $minutesToAdd);
                                            }
                                            ...
                                            foreach ($arMinutes as $minute) {
                                                ...
                                                <div class="hour">
                                                    <?if($reserv == true || $dateTime < $currentDateTime):?>
                                                        <?=$minute?>
                                                    <?else:?>
                                                        <a data-type="registry" data-date="<?=$date['DATE']?>" data-time="<?=$minute?>" data-topic="<?=$regDataProps['TOPICS']?>" class="btn-schedule-hour">
                                                            <?=$minute?>
                                                        </a>
                                                    <?endif;?>
                                                </div>
