...
$arElement = false;
$arParams["IBLOCK_ID"] = (int)$iblock_obj = \Bitrix\Iblock\IblockTable::getList(['filter' => ['=CODE' => $arParams["IBLOCK_CODE"]], 'select' => ['ID']])->fetch()['ID'];

if(is_array($arParams["PROPERTY_CODES"]))
{
    foreach ($arParams["PROPERTY_CODES"] as $key => $value){
        if (strpos($value, '_') === 0){
            $arParams["PROPERTY_CODES"][$key] = (int)\Bitrix\Iblock\PropertyTable::getList(['filter' => ['=CODE' => ltrim($value, "_"), '=IBLOCK_ID' => $arParams["IBLOCK_ID"]],'select' => ['ID']])->fetch()["ID"];
        }
    }
}
if(is_array($arParams["PROPERTY_CODES_REQUIRED"]))
{
    foreach ($arParams["PROPERTY_CODES_REQUIRED"] as $key => $value){
        if (strpos($value, '_') === 0){
            $arParams["PROPERTY_CODES_REQUIRED"][$key] = (int)\Bitrix\Iblock\PropertyTable::getList(['filter' => ['=CODE' => ltrim($value, "_"), '=IBLOCK_ID' => $arParams["IBLOCK_ID"]],'select' => ['ID']])->fetch()["ID"];
        }
    }
}

if(is_array($arParams["PROPERTY_VALIDATE_TYPE"]))
{
    foreach ($arParams["PROPERTY_VALIDATE_TYPE"] as $key => $value){
        if (strpos($key, '_') === 0){
            $propId = (int)\Bitrix\Iblock\PropertyTable::getList(['filter' => ['=CODE' => ltrim($key, "_"), '=IBLOCK_ID' => $arParams["IBLOCK_ID"]],'select' => ['ID']])->fetch()["ID"];
            $arParams["PROPERTY_VALIDATE_TYPE"][$propId] = $value;
        }
    }
}
...
if ($bAllowAccess)
{
    ...
    foreach ($arResult["DEF_VALUES"] as $key => $arr)
    {
    ...
    if(in_array("NAME", $arResult["PROPERTY_LIST"])){
        $arResult["PROPERTY_LIST"] = array_diff($arResult["PROPERTY_LIST"], array("NAME"));
        array_unshift($arResult["PROPERTY_LIST"], "NAME");
    }
...
if ($bAllowAccess)
{
	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
        ...
        {
            $arResult["use_consent"] = $_REQUEST["use_consent"];
            if ($_REQUEST["use_consent"] != "on")
            {
                $arResult["ERRORS"][] = GetMessage("IBLOCK_ADD_ERROR_use_consent");
            }
        }
        ...
        foreach ($arParams["PROPERTY_VALIDATE_TYPE"] as $propertyID => $validateType){
            $vError = false;
            $propertyValue = intval($propertyID) > 0 ? $arUpdatePropertyValues[$propertyID] : $arUpdateValues[$propertyID];
            if (!is_array($propertyValue))
            {
				if($propertyValue != '' && strlen($propertyValue) > 0)
                    $vError = true;
			}
            if ($vError){
                switch ($validateType){
                    case "phone":
                        $phoneNumber = str_replace(['(', ')', ' ', '-'], '', $propertyValue);
                        if(!preg_match("/^\+7\d{10}$/", $phoneNumber)){
                            $arResult["ERRORS"][] = str_replace("#PROPERTY_NAME#", intval($propertyID) > 0 ? $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] : (!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)), GetMessage("IBLOCK_ADD_ERROR_VALIDATE_TYPE_phone"));
                        }
                        break;
                    case "text":
                        $propertyValue = preg_replace('/\s+/', ' ', $propertyValue);
                        if (!preg_match("#^[a-zA-Z0-9а-яA-ЯёЁ\-,/!:?;.\s]+$#",$propertyValue)) {
                            $arResult["ERRORS"][] = str_replace("#PROPERTY_NAME#", intval($propertyID) > 0 ? $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] : (!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)), GetMessage("IBLOCK_ADD_ERROR_VALIDATE_TYPE_text"));
                        }
                        break;
                    case "text_length":
                        $propertyValue = preg_replace('/\s+/', ' ', $propertyValue);
                        ...
                        if (mb_strlen($propertyValue) < 100){
                            $arResult["ERRORS"][] = str_replace("#PROPERTY_NAME#", intval($propertyID) > 0 ? $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] : (!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)), GetMessage("IBLOCK_ADD_ERROR_VALIDATE_TYPE_text_length"));
                        }
                        break;
                    case "text_length_30":
                        $propertyValue = preg_replace('/\s+/', ' ', $propertyValue);
                        ...
                        if (mb_strlen($propertyValue) < 30){
                            $arResult["ERRORS"][] = str_replace("#PROPERTY_NAME#", intval($propertyID) > 0 ? $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] : (!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)), GetMessage("IBLOCK_ADD_ERROR_VALIDATE_TYPE_text_length_30"));
                        }
                        break;
                    case "email":
                        if (!filter_var($propertyValue, FILTER_VALIDATE_EMAIL)){
                            $arResult["ERRORS"][] = str_replace("#PROPERTY_NAME#", intval($propertyID) > 0 ? $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] : (!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)), GetMessage("IBLOCK_ADD_ERROR_VALIDATE_TYPE_string"));
                        }
                        break;
                    case "string":
                        if (!preg_match("/^[a-zA-Zа-яёА-ЯЁ\-]+$/u", $propertyValue)) {
                            $arResult["ERRORS"][] = str_replace("#PROPERTY_NAME#", intval($propertyID) > 0 ? $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] : (!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)), GetMessage("IBLOCK_ADD_ERROR_VALIDATE_TYPE_string"));
                        }
                        break;
                }
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			if ($arParams["ID"] > 0)
			{
				...
				while($arPropV = $dbPropV->Fetch())
				{
                    ...
                    if ($arPropV["CODE"] == "OPERATOR_LINK" && !empty($arPropV["VALUE"])){
                        $update = false;
                    }

                    if ($arPropV["CODE"] == "STATUS" && $arPropV["VALUE_XML_ID"] != "new"){
                        $update = false;
                    }
                ...
                if(!$update){
                    $arResult["ERRORS"][] = "Заявка принять на работу";
                } else if (!$res = $oElement->Update($arParams["ID"], $arUpdateValues, $bWorkflowIncluded, true, $arParams["RESIZE_IMAGES"]))
				{
					$arResult["ERRORS"][] = $oElement->LAST_ERROR;
				}
                else if (strlen($arParams["LIST_URL"]) > 0 && $arParams["ID"] > 0){
	...
	if ($bShowForm)
	{
		...
		$arResult["MESSAGE"] = '';
		if (isset($_SESSION["NEW_ELEMENT"]) && $_SESSION["NEW_ELEMENT"] == "Y"){
            $arResult["MESSAGE"] = $arParams["USER_MESSAGE_ADD"];
            $_SESSION["NEW_ELEMENT"] = "N";
        }
