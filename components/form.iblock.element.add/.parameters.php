...
{
    $arCurrentValues["IBLOCK_ID"] = (int)$iblock_obj = \Bitrix\Iblock\IblockTable::getList(['filter' => ['=CODE' => $arCurrentValues["IBLOCK_CODE"]], 'select' => ['ID']])->fetch()['ID'];
...
while ($arr=$rsProp->Fetch())
{
	...
	{
		$arProperty_LNSF["_".$arr["CODE"]] = "[".$arr["CODE"]."] ".$arr["NAME"];
	}
}
...
$arComponentParameters["PARAMETERS"]["USE_USER_CONSENT"] = array(
    "PARENT" => "PARAMS",
    "NAME" => GetMessage("USE_USER_CONSENT"),
    "TYPE" => "CHECKBOX",
    "DEFAULT" => "N",
);
