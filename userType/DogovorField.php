...
class DogovorField extends \Bitrix\Main\UserField\TypeBase
{

    const USER_TYPE_ID = 'dogovor_field';
    ...
    public static function GetPublicView($arUserField, $arAdditionalParameters = array())
    {
        ...
        foreach($value as $res)
        {
            ...
            $dogovor = static::GetDogovor($res);

            $res = '<a target="blank" href="/services/lists/28/element/0/'.$res.'/">'.$dogovor['NAME'].'</a>';
    ...
    static function GetDogovor($id)
    {
        \CModule::IncludeModule('iblock');
        $dogovor = \Bitrix\Iblock\ElementTable::GetList(array(
            'select' => array('NAME'),
            'filter' => array('IBLOCK_ID' => 28, 'ID' => (int)$id)
        ))->fetch();
