...
class TasksImportComponent extends TasksBaseComponent
{
    ...
    private function handleStep1(HttpRequest $request): void
    {
        ...
        $customImport = $request->get('hidden_custom_import');
        if (empty($format) && $customImport) {
            $format = 'csv';
        }

        if ($customImport == "true") {
            if (CModule::IncludeModule('rns.integrations')) {
                $importer = new Import();
                try {
                    $file = $request->getFile('file');

                    $destPath = $_SERVER['DOCUMENT_ROOT'] . '/upload/' . $file['name'];

                    move_uploaded_file($file['tmp_name'], $destPath);
                    $this->arResult['IMPORT_FILE_PARAMETERS']['CUSTOM_FILE_PATH'] = $destPath;

                    $projectId = $request->get('project_id');
                    $this->arResult['IMPORT_FILE_PARAMETERS']['PROJECT_ID'] = $projectId;

                    $importer->importFile($format, $destPath, '', intval($projectId), true);

                    $this->arResult['IMPORT_FILE_PARAMETERS']['FORMAT'] = $format;
                    $this->arResult['IMPORT_FILE_PARAMETERS']['HEADERS'] = $importer->getHeaders();
                    $this->arResult['IMPORT_FILE_PARAMETERS']['ROWS'] = $importer->getData();
                    $this->arResult['IMPORT_FILE_PARAMETERS']['SKIPPED_COLUMNS'] = [];
                    $this->arResult['STEP'] = 2;
                } catch (\Exception $ex) {
                    $this->arResult['ERRORS']['FILE_LABEL'] = $ex->getMessage();
                }
                return;
            }
        }
    ...
    private function handleStep2Next(HttpRequest $request): void
    {
        ...
        if ($customImport == 'true') {
            if (CModule::IncludeModule('rns.integrations')) {
                $this->arResult['IMPORT_FILE_PARAMETERS']['FORMAT'] = $format;
                $this->arResult['IMPORT_FILE_PARAMETERS']['CUSTOM_FILE_PATH'] = $request->get('hidden_custom_file_path');
                $this->arResult['IMPORT_FILE_PARAMETERS']['PROJECT_ID'] = intval($request->get('hidden_project_id'));
                $this->arResult['CURRENT_USER_ID'] = $this->arParams['VARIABLES']['user_id'];
                $this->arResult['IMPORT_FILE_PARAMETERS']['FILE_POS'] = 0;
                $this->arResult['IMPORT_FILE_PARAMETERS']['MAX_EXECUTION_TIME'] =
                    Option::get('tasks', 'import_step', 5);
