BX.TasksImport = (function ()
{
	...
	TasksImport.prototype.validateFile = function(file)
	{
		...
		var projectNotSelectedText = BX.message('TASKS_IMPORT_ERROR_PROJECT_NOT_SELECTED');

		if (this.parameters.customImport && !Number(BX('project_id').value)) {
			this.fileErrors = {PROJECT_LABEL: projectNotSelectedText};
		...
		else
		{
			...
			if (!(fileExtension === 'csv' || fileExtension === 'mpp' || fileExtension === 'mpx')) {
			...
			if (this.parameters.fileMaxSize > 0 && size > this.parameters.fileMaxSize) {
				this.fileErrors = {
					FILE_LABEL: 'Размер файла (' + size.toFixed(0) + ' Мб) превышает максимально допустимый ' + this.parameters.fileMaxSize + ' Мб.'
				};
				TasksImport.prototype.showErrors(this.fileErrors);
				return false;
			}
	...
	TasksImport.prototype.startImport = function(parameters)
	{
		var promise;
		var self = this;
		if (this.parameters.customImport) {
			promise = BX.ajax.runAction('rns:integrations.api.task.import', {
				data: {
					importParameters: parameters
				}
			});
		} else {
			promise = BX.ajax.runComponentAction('bitrix:tasks.import', 'startImport', {
				mode: 'ajax',
				data: {
					importParameters: parameters
				}
			...
			if (successfulImports > 0 || errorImports > 0)
			{
				...
				if (!data['ALL_LINES_LOADED'])
				{
					if (BX('hidden_force_import_stop').value === 'N')
					{
						...
						TasksImport.prototype.parameters = self.parameters;
