			...
			getSelector: function()
			{
				return this.subInstance('selector', function(){
					...
					var options = {
						...
						userRight: this.option('userRight'),
	...
	BX.Tasks.Component.TasksWidgetMemberSelector.ItemManager = BX.Tasks.UserItemSet.extend({
		...
		methods: {
			...
			getDialog: function()
			{
				...
				let  dialogparams = this.getDialogParams();

				this.dialog = new BX.UI.EntitySelector.Dialog(dialogparams);
			...
			{
				let params = {};
				let userRight = this.option('userRight');
				let role = userRight.role;
				let users = userRight.users;
				let usersID = [];


				if(role === "WRITE"){
					params.enableSearch = true;
					params.multiple = this.option('max') > 1;
					params.context = 'TASKS_MEMBER_SELECTOR_EDIT_' + this.option('userType');
					params.entities = this.getDialogEntities();
					params.preselectedItems = this.getDialogSelectedItems();
				}else if (role === "READE"){
					...
					for (var key in users) {
						...
						if (users.hasOwnProperty(key)) {
							...
							item.avatar = users[key].avatar;
							item.title = users[key].title;
							item.entityId = 'user';
							item.tabs = 'users';
							items.push(item);
						}
					...
					params.items = items;
					params.tabs = [
						{ id: 'users', title: 'Подчиненные' }
					];

					params.showAvatars = true;
					params.dropdownMode = true;
					params.multiple = this.option('max') > 1;
					params.preselectedItems = this.getDialogSelectedItems();
				}else{
					params.enableSearch = false;
