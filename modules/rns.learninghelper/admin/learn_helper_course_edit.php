...
$isCopy = (isset($_REQUEST["action"]) && $_REQUEST["action"] == "copy") ? true : false;
$bDenyAutosave = false;
if ($COURSE_ID !== 0)
{
	...
	else
	{
		...
		$aContext = array(
			array(
				...
				"LINK"=>"learn_helper_unilesson_admin.php?lang=" . LANG . '&PARENT_LESSON_ID=-1' . GetFilterParams("filter_"),
...
if ( ($_SERVER["REQUEST_METHOD"] == "POST") && ($Perm >= "X") && ($_POST["Update"] <> '') && check_bitrix_sessid() )
{
	...
	{
        $rsData = CUserTypeEntity::GetList( array($by=>$order), array('ENTITY_ID' => 'LEARNING_LESSONS', 'USER_TYPE_ID' => 'file',) );
        while($arRes = $rsData->Fetch())
        {
            if(!array_key_exists($arRes["FIELD_NAME"], $arFields))
                continue;

            if($arRes["MULTIPLE"] == "Y"){
                $n = 0;
                foreach($arFields[$arRes["FIELD_NAME"]] as $key => $item){
                    if($fileId = (int)$item["old_id"])
                    {
                        $file = CFile::MakeFileArray(CFile::GetPath($fileId));
                        $arFields[$arRes["FIELD_NAME"]][$key] = $file;
                    }
                }
            }else{
                if($fileId = (int)$arFields[$arRes["FIELD_NAME"]]["old_id"])
                {
                    $file = CFile::MakeFileArray(CFile::GetPath($fileId));
                    $arFields[$arRes["FIELD_NAME"]] = $file;
                }
            }
        }
		...
		if ($oAccess->IsBaseAccess(CLearnAccess::OP_LESSON_CREATE))
		{
			...
			$lesson_ID = CCourse::CourseGetLinkedLesson($COURSE_ID);
	...
	try
	{
        ...
        if ($isCopy && $lesson_ID)
            CLearnRelationHelper::ProccessPOST($oAccess, $lesson_ID);
	}
	catch (Exception $e)
	{
		$res = false;
	...
	else
	{
		...
		if(!$bVarsFromForm)
		{
			if($apply == '')
			{
				...
				{
					LocalRedirect("/bitrix/admin/learn_helper_unilesson_admin.php?lang=" . LANG
						. $returnPath
						. GetFilterParams("filter_", false));
				}
			}

			LocalRedirect("/bitrix/admin/learn_helper_course_edit.php?lang=".LANG
				. $returnPath
				. "&COURSE_ID=".$COURSE_ID
...
$aContext = array(
	array(
		...
		"LINK"=>"learn_helper_unilesson_admin.php?lang=" . LANG."&PARENT_LESSON_ID=-1"
			. $returnPath
...
if ($Perm >= "X" && $COURSE_ID > 0)
{
	$aContext[] = 	array(
		...
		"LINK"=>"learn_helper_course_edit.php?lang=".LANG.GetFilterParams("filter_"),
	...
	if ($linkedLessonId !== false)
	{
		$aContext[] = array(
			...
			"LINK"	=> "javascript:if(confirm('".GetMessage("LEARNING_CONFIRM_DEL_MESSAGE")
				."'))window.location='learn_helper_unilesson_admin.php?action=delete&ID=". $linkedLessonId
				."&lang=".LANG
				. $returnPathUrlencoded
...
$tabControl->Buttons(
	array(
		...
		"back_url" => "/bitrix/admin/learn_helper_unilesson_admin.php?lang=".LANG."&PARENT_LESSON_ID=-1"
			. $uriParentLessonPath
			. GetFilterParams("filter_", false)
		)
	);
...
$tabControl->arParams["FORM_ACTION"] .= $isCopy ? '&action=copy':'';
$tabControl->Show();
