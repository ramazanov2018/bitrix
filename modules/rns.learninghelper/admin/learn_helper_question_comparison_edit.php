...
{
	$result = ComparisonAnswer::GetByID($ID);
	$arData = $result->Fetch();
...
if (!$bBadCourse && $_SERVER["REQUEST_METHOD"] == "POST" && $Update <> '' && check_bitrix_sessid())
{
	...
	else
	{
		...
		if (!$bVarsFromForm)
		{
			...
			LocalRedirect("/bitrix/admin/learn_helper_question_comparison_edit.php?lang=" . LANG
				. '&LESSON_PATH=' . $uriLessonPath
				. "&ID=" . $ID
...
if ($ID > 0)
{
	$arContextPopup = Array(
        ...
        Array(
			"TEXT"   => GetMessage('LEARNING_COMPARISON_ANSWER'),
			"ACTION" => "window.location='learn_helper_question_comparison_edit.php?lang=" . LANG
				. '&LESSON_PATH=' . $uriLessonPath
				. GetFilterParams("filter_", false)
				. "&QUESTION_TYPE=C"
				. ($from=="learn_admin"?"&from=learn_admin":"")
	...
	<tr>
		...
		<td width="60%">
			<select onchange="if(this[this.selectedIndex].value!='') window.location=this[this.selectedIndex].value;"<?php echo (defined('BX_PUBLIC_MODE') && BX_PUBLIC_MODE == 1 ? "disabled=\"disabled\"" : "")?>>
				...
				<option value="<?=$APPLICATION->GetCurPageParam("QUESTION_TYPE=C", array("QUESTION_TYPE", "tabControl_active_tab"))?>" <?if($str_QUESTION_TYPE=="C") echo "selected"?>><?echo GetMessage("LEARNING_COMPARISON_ANSWER")?></option>
			</select>
