...
use LearningHelper\ComparisonAnswer;
use LearningHelper\HelperCLQuestion;

require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_before.php");

if (!CModule::IncludeModule('learning') || !CModule::IncludeModule('rns.learninghelper'))
{
...
CModule::IncludeModule('rns.learninghelper');
...
{
	$result = HelperCLQuestion::GetByID($ID);
	$arData = $result->Fetch();
...
if (!$bBadCourse && $_SERVER["REQUEST_METHOD"] == "POST" && $Update <> '' && check_bitrix_sessid())
{
	...
	{
        if($QUESTION_TYPE == ComparisonAnswer::CUESTION_TYPE){
            $answers = ComparisonAnswer::GetList(Array(),Array("QUESTION_ID" => $ID));

            while ($a = $answers->GetNext())
            {
                //delete?
                if (${"ANSWER_".$a["ID"]."_DEL"} == "Y" || ${"ANSWER_".$a["ID"]."_ANSWER"} == '' && ${"ANSWER_".$a["ID"]."_QUESTION"} == '')
                {
                    if(!ComparisonAnswer::Delete($a["ID"]))
                    {
                ...
                else
                {
                    ...
                    $arFields = Array(
                        ...
                        "QUESTION" => ${"ANSWER_".$a["ID"]."_QUESTION"},
                    );

                    $asw = new ComparisonAnswer;
            ...
            for ($i=0; $i<500; $i++)
            {
                if (${"ANSWER_n".$i."_ANSWER"} == '' && ${"ANSWER_n".$i."_QUESTION"} == '') continue;

                $arFields = Array(
                    "SORT" => ${"ANSWER_n".$i."_SORT"},
                    "ANSWER" => ${"ANSWER_n".$i."_ANSWER"},
                    "QUESTION" => ${"ANSWER_n".$i."_QUESTION"},
                    "QUESTION_ID" => $ID,
                );
		...
		if (!$bVarsFromForm)
		{
			if($apply == '')
			{
				...
				{
					LocalRedirect("/bitrix/admin/learn_helper_question_admin.php?lang=" . LANG
			...
			LocalRedirect("/bitrix/admin/learn_helper_question_edit.php?lang=" . LANG
				. '&LESSON_PATH=' . $uriLessonPath
				. "&ID=" . $ID
...
$aContext = array(
	array(
		...
		"LINK"  => "learn_helper_question_admin.php?lang=" . LANG
			. '&LESSON_PATH=' . $uriLessonPath
...
if ($ID > 0)
{
	$arContextPopup = Array(
		Array(
			...
			"ACTION" => "window.location='learn_helper_question_edit.php?lang=" . LANG
				. '&LESSON_PATH=' . $uriLessonPath
				. GetFilterParams("filter_", false)
				. "&QUESTION_TYPE=S"
				. ($from=="learn_admin"?"&from=learn_admin":"")
	...
	$aContext[] = 	array(
		...
		"LINK" => "javascript:if(confirm('" . GetMessage("LEARNING_CONFIRM_DEL_MESSAGE")
			. "'))window.location='learn_helper_question_admin.php?lang=" . LANG
			. '&LESSON_PATH=' . $uriLessonPath
			. "&action=delete&ID=" . $ID
			. "&" . bitrix_sessid_get()
	...
	<tr>
		...
		<td width="60%">
			<select onchange="if(this[this.selectedIndex].value!='') window.location=this[this.selectedIndex].value;"<?php echo (defined('BX_PUBLIC_MODE') && BX_PUBLIC_MODE == 1 ? "disabled=\"disabled\"" : "")?>>
				...
				<option value="<?=$APPLICATION->GetCurPageParam("QUESTION_TYPE=C", array("QUESTION_TYPE", "tabControl_active_tab"))?>" <?if($str_QUESTION_TYPE==ComparisonAnswer::CUESTION_TYPE) echo "selected"?>><?echo GetMessage("LEARNING_COMPARISON_ANSWER")?></option>
			</select>
...
<?php $tabControl->BeginCustomField("ANSWERS", GetMessage("LEARNING_ANSWERS"), false);?>
	<tr>
		<td valign="top" colspan="2">
			<?php
			if($str_QUESTION_TYPE == ComparisonAnswer::CUESTION_TYPE):?>
                <table cellpadding="0" cellspacing="0" width="100%" class="internal" id="answers-table">
                    ...
                    <tr class="heading">
                        ...
                        <td align="center"><?echo GetMessage("LEARNING_QUESTION_TABLE")?></td>
                    ...
                    function _HGetOldAndNew($answers)
                    {
                        global $arNewIDs;
                    ...
                    $Canswers = ComparisonAnswer::GetList(Array("SORT" => "ASC","ID" => "ASC"),Array("QUESTION_ID"=>$ID));
                    //$answers = CLAnswer::GetList(Array("SORT" => "ASC","ID" => "ASC"),Array("QUESTION_ID"=>$ID));
                    while ($a = _HGetOldAndNew($Canswers)):
                        if ($bVarsFromForm)
                        {
                            $DB->InitTableVarsForEdit("b_learn_h_comparison_answer", "ANSWER_".$str_ANSWER_ID."_", "str_ANSWER_");
                        ...
                        <tr>
                            ...
                            <td align="center" style="text-align:center;">
                                <textarea style="width: 98%" name="ANSWER_<?=$str_ANSWER_ID?>_QUESTION"><?php echo $str_ANSWER_QUESTION; ?></textarea>
                            </td>
                            <td align="center" style="text-align:center;">
                    ...
                    function addAnswer() {
                        ...
                        var row = BX.create("tr", {
                            children: [
                                ...
                                BX.create('td', {
                                    html : '<textarea name="ANSWER_n' + nextNum + '_QUESTION" style="width: 98%"></textarea>',
                                    props : {align: 'center'}
                                }),
                                BX.create('td', {
                ...
                <br />
                <a href="javascript:void(0)" class="adm-btn" onclick="addAnswer();"><?php echo GetMessage("LEARNING_ADD_QUESTION")?></a>
            <?elseif ($str_QUESTION_TYPE != "T"):?>
				<table cellpadding="0" cellspacing="0" width="100%" class="internal" id="answers-table">
					<tbody id="answers-table-tbody">
					<tr class="heading">
						<td align="center" width="40">ID</td>
                        <td align="center" width="40"><?echo GetMessage("LEARNING_QUESTION_ADM_CORRECT")?></td>
						<td align="center"><?echo GetMessage("LEARNING_ANSWER")?></td>
						<td align="center" width="40"><?echo GetMessage("LEARNING_COURSE_ADM_SORT")?></td>
						<td align="center" width="40"><?echo GetMessage("LEARNING_COURSE_ADM_DELETE")?></td>
					</tr>
...
$tabControl->Buttons(
	Array("back_url" =>
		...
		:
		"learn_helper_question_admin.php?lang=".LANG . '&LESSON_PATH=' . $uriLessonPath . GetFilterParams("filter_", false)
	)
);
