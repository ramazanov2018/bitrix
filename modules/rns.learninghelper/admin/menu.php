...
function GetLinkedCourse($lessonId){
    $arMap = \CLearnLesson::GetCourseToLessonMap();

    if (isset($arMap['L' . $lessonId]) )
        return ($arMap['L' . $lessonId]);

    if (isset($arMap['C' . $lessonId]) )
        return ($arMap['C' . $lessonId]);

    // return id of corresponded course
    return (false);
}

function __learning_helper_admin_get_menu ($THIS, $arPath = array(), $deep = 0, &$immediateChildsIds, $loadOnlySpecialEntities = false)
{
	...
	else
	{
		...
		$arMenu[] = array(
			...
			"url" => "learn_helper_question_admin.php?lang=" . LANG
			...
			"more_url" =>
				array(
					"learn_helper_question_admin.php?lang=" . LANG
						. '&PARENT_LESSON_ID=' . ($parentLessonId + 0)
						. '&LESSON_PATH=' . $urlPath
						. "&from=learn_menu",
					"learn_helper_question_edit.php?lang=" . LANG
						. '&LESSON_PATH=' . $urlPath
						. "&from=learn_menu",
					"learn_helper_question_admin.php?lang=" . LANG
						. '&PARENT_LESSON_ID=' . ($parentLessonId + 0)
		...
		{
            $NewparentLessonId = $arPath[0];
			$immediateParentCourse = CLearnLesson::GetLinkedCourse ($NewparentLessonId);
			if ($immediateParentCourse !== false)
			{
				// immediate parent is a course, so we must add entity 'Tests'
				$arMenu[] = array(
					'text'      => GetMessage('LEARNING_TESTS'),
					'url'       => 'learn_helper_test_admin.php?lang=' . LANG
					...
					'more_url'  =>
						array(
							'learn_helper_test_admin.php?lang=' . LANG
								. '&set_filter=Y'
								. '&COURSE_ID=' . ($immediateParentCourse + 0)
								. '&PARENT_LESSON_ID=' . ($parentLessonId + 0)
								. '&LESSON_PATH=' . $urlPath,
							'learn_helper_test_edit.php?lang=' . LANG
								. '&filter=Y&set_filter=Y'
								. '&COURSE_ID=' . ($immediateParentCourse + 0)
								. '&PARENT_LESSON_ID=' . ($parentLessonId + 0)
								. '&LESSON_PATH=' . $urlPath
	...
	while ($arData = $CDBResult->GetNext())
	{
		...
		if ($deep === 0)
		{
			if ($items >= $learning_menu_max_courses)
			{
				...
				$arMenu[] = array(
					...
					'url'      => 'learn_helper_unilesson_admin.php?lang=' . LANG . '&PARENT_LESSON_ID=-1',
					'title'    => GetMessage('LEARNING_MENU_COURSES_ALT'),
					'more_url' => array(
						'learn_helper_test_admin.php',
						'learn_helper_test_edit.php',
						'learn_helper_unilesson_admin.php',
						'learn_unilesson_edit.php',
						'learn_helper_question_admin.php',
						'learn_helper_question_edit.php'
						)
					);
		...
		$arItem = array(
			'text'      => $arData['NAME'],
			'url'       => 'learn_helper_unilesson_admin.php?lang=' . LANG
			...
			'module_id' => 'rns.learninghelper',
			'more_url'  => array(
				'learn_helper_unilesson_admin.php?lang=' . LANG
					. '&set_filter=Y'
					. '&PARENT_LESSON_ID=' . ($arData['LESSON_ID'] + 0)
		...
		if ( ( $deep === 0) && isset($arData['LINKED_LESSON_ID']) && ($arData['LINKED_LESSON_ID'] > 0 ) )
		{
			...
			if ($arData['LESSON_ID'] == $currentLessonIdInPath)
				$arItem['items'] = __learning_helper_admin_get_menu ($THIS, $arPath, $deep + 1, $arSubImmediateChildsIds);
		}
		else
		{
			$loadOnlySpecialEntities = false;
...
if (\CLearnAccessMacroses::CanViewAdminMenu())
{
	...
	elseif (isset($_GET['admin_mnu_module_id'])
		...
		&& ($_GET['admin_mnu_module_id'] === 'rns.learninghelper')
	)
	{
	...
	$aMenu = array(
		...
		Array(
			...
			array(
				...
				"items"     => __learning_helper_admin_get_menu($this, $arPath, $deep = 0, $arSubImmediateChildsIds)	// Build menu from the top
			),
			...
			array(
				...
				"more_url" =>
					Array(
						...
						"learn_helper_test_result_edit.php",
						"learn_helper_test_result_admin.php",
					),
			)
		)
	);
	...
	if ($oAccess->IsBaseAccess(CLearnAccess::OP_LESSON_MANAGE_RIGHTS))
	{
		$aMenu['items'][] = array(
			...
			'url'       => 'learn_helper_group_admin.php?lang=' . LANG,
        ...
        $aMenu['items'][] = array(
			'text'      =>  GetMessage('LEARNING_MENU_GROUPS_WAITLIST'),
			'url'       => 'learn_helper_group_waitlist_admin.php?lang=' . LANG,
			'title'     =>  GetMessage('LEARNING_MENU_GROUPS_ALT'),
			'items_id'  => 'menu_learning_groups',
			'icon'      => 'learning_icon_groups',
			//'page_icon' => 'learning_page_icon_export',
