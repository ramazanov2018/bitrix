...
if (!CModule::IncludeModule('rns.learninghelper'))
{
	require($_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/main/include/prolog_admin_after.php'); // second system's prolog
...
if (isset($_REQUEST['IMPORT']) && $_REQUEST['IMPORT'] == 'WAITLIST'){
    \LearningHelper\WaitList::WaitListImport($ID);
    LocalRedirect($APPLICATION->GetCurPageParam("", array("IMPORT")));
}
...
if($bBadResult)
{
	...
	$aContext = array(
		array(
			...
			"LINK"=>"learn_helper_group_admin.php?lang=" . LANG,
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $Update <> '' && check_bitrix_sessid())
{
    ...
    $entity = HighloadBlockTable::compileEntity(\Rns\Dictionary::COURSE_MODULE_STREAM_DATES);
    $entityClass = $entity->getDataClass();

    foreach ($_POST as $key => $value) {
        $matches = [];
        if (preg_match('/ACTIVE_FROM_MODULE_(?<courseID>\d*)_(?<moduleID>\d*)/', $key, $matches)) {
            $adModuleDates = $entityClass::getList(array(
                "select" => array("*"),
                "order" => array("ID" => "ASC"),
                "filter" => array("UF_COURSE_ID" => $matches['courseID'], 'UF_MODULE_ID' => $matches['moduleID'], 'UF_STREAM_ID' => $_POST['ID'])
            ))->Fetch();
            if (!$adModuleDates) {
                $adModuleDates = [
                    "UF_COURSE_ID" => $matches['courseID'],
                    'UF_MODULE_ID' => $matches['moduleID'],
                    'UF_STREAM_ID' => $_POST['ID']
                ];
                $adModuleDates['UF_ACTIVE_FROM'] = $_POST['ACTIVE_FROM_MODULE_' . $matches['courseID'] . '_' . $matches['moduleID']];
                $adModuleDates['UF_ACTIVE_TO'] = $_POST['ACTIVE_TO_MODULE_' . $matches['courseID'] . '_' . $matches['moduleID']];
                $entityClass::Add($adModuleDates);
            } else {
                $adModuleDates['UF_ACTIVE_FROM'] = $_POST['ACTIVE_FROM_MODULE_' . $matches['courseID'] . '_' . $matches['moduleID']];
                $adModuleDates['UF_ACTIVE_TO'] = $_POST['ACTIVE_TO_MODULE_' . $matches['courseID'] . '_' . $matches['moduleID']];
                $entityClass::Update($adModuleDates['ID'], $adModuleDates);
            }
	...
	else
	{
		...
		if($apply == '')
		{
			if($return_url <> '')
				...
				LocalRedirect("/bitrix/admin/learn_helper_group_admin.php?lang=".LANG.GetFilterParams("filter_", false));
		}

		LocalRedirect("/bitrix/admin/learn_helper_group_edit.php?lang=".LANG."&ID=".$ID.GetFilterParams("filter_", false));
	}
}
...
$adminChain->AddItem(array(
	...
	"LINK"=>"learn_helper_group_admin.php?lang=". LANG.GetFilterParams("filter_", false)
));
...
if ($str_COURSE_LESSON_ID) {
    ...
    $obModules = CLearnLesson::GetListOfImmediateChilds($str_COURSE_LESSON_ID);
$entity = HighloadBlockTable::compileEntity(\Rns\Dictionary::COURSE_MODULE_STREAM_DATES);
$entityClass = $entity->getDataClass();
$adModuleDates = $entityClass::getList(array(
    "select" => array("*"),
    "order" => array("ID" => "ASC"),
    "filter" => array("UF_COURSE_ID" => $str_COURSE_LESSON_ID, 'UF_STREAM_ID' => $str_ID)
));
$arModules = [];
while ($arModuleData = $adModuleDates->Fetch()) {
    $arModules[$arModuleData['UF_MODULE_ID']] = $arModuleData;
}

while ($arModule = $obModules->Fetch()) {
        $tabControl->AddSection("LEARNING_ACTIVATION_MODULE_". $arModule['LESSON_ID'], $arModule['NAME']);
        $tabControl->BeginCustomField("MODULES" . $arModule['LESSON_ID'], "Даты начала/конца", $required = true);
        $name = $str_COURSE_LESSON_ID . '_' . $arModule['LESSON_ID'];
        $fromNameValue = $arModules[$arModule['LESSON_ID']]['UF_ACTIVE_FROM'];
        $toNameValue = $arModules[$arModule['LESSON_ID']]['UF_ACTIVE_TO'];

    ?>
    <tr>
        <td><?php echo $tabControl->GetCustomLabelHTML()?>:</td>
        <td>
            <?echo CalendarPeriod("ACTIVE_FROM_MODULE_" . $name, $fromNameValue, "ACTIVE_TO_MODULE_" . $name, $toNameValue, "learningGroupResultTabControl", "N", "", "", "19")?>
        </td>
    </tr>

    <?
        $tabControl->EndCustomField("MODULES" . $arModule['LESSON_ID']);
    }
} else {
    $tabControl->AddSection("MODULES", 'Сохраните поток перед вводом дат модулей');
}
//end Даты модулей
...
$tabControl->Buttons(Array("back_url" =>"learn_helper_group_admin.php?lang=". LANG.GetFilterParams("filter_", false)));
