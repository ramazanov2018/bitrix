...
if($bBadResult)
{
	...
	$aContext = array(
		array(
			...
			"LINK"=>"learn_helper_group_waitlist_admin.php?lang=" . LANG,
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $Update <> '' && check_bitrix_sessid())
{
	...
	if ($isAccessible)
	{
		...
		if ($ID == 0)
		{
            $arFields['COURSE_LESSON_ID'] = \LearningHelper\WaitList::COURSE_WAITLIST;
	...
	else
	{
		...
		if($apply == '')
		{
			if($return_url <> '')
				...
				LocalRedirect("/bitrix/admin/learn_helper_group_waitlist_admin.php?lang=".LANG.GetFilterParams("filter_", false));
		}
...
$adminChain->AddItem(array(
	...
	"LINK"=>"learn_helper_group_waitlist_admin.php?lang=". LANG.GetFilterParams("filter_", false)
));
...
$tabControl->Buttons(Array("back_url" =>"learn_helper_group_waitlist_admin.php?lang=". LANG.GetFilterParams("filter_", false)));
