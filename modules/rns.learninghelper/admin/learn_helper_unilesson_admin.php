...
class CLearnRenderAdminUnilessonList
{
	...
	public function BuildList()
	{
		...
		while ($arRes = $this->rsData->NavNext(false))	// NavNext: don't extract fields to globals
		{
			...
			if ($courseId !== false)
			{
				$hrefPrefix = 'learn_helper_course_edit.php?lang='.LANG.'&COURSE_ID=' . $courseId;
			...
			{
				$hrefPrefix = 'learn_helper_unilesson_edit.php?lang='.LANG.'&LESSON_ID=' . $arRes['LESSON_ID']
			...
			$actionCopyLesson = $actionEditLesson.'&action=copy';

			$rowAction = false;
			$rowTitle = '';
			if ( ! $this->IsSearchMode() )
			{
				$rowAction = 'learn_helper_unilesson_admin.php?lang=' . LANG
			...
			$htmlChapters  = (string) ((int) $chapterCount)
				. '&nbsp;[<a href="learn_helper_unilesson_edit.php?lang=' . LANG
			...
			if ( ! $this->IsSearchMode() )
			{
                ...
                $arActions[] = array(
					...
					'ACTION'  => $this->oList->ActionRedirect($actionCopyLesson)
				);
			...
			if ($isAccessCreate)
			{
				...
				$aContext[] =
					array(
						...
						'LINK'  => 'learn_helper_course_edit.php?lang=' . LANG
							. $PROPOSE_RETURN_LESSON_PATH,
				...
				$aContext[] =
					array(
						...
						'LINK'  => 'learn_helper_unilesson_edit.php?lang=' . LANG
