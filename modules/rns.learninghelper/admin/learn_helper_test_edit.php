...
$lessonId = 0;
$PARENT_LESSON_ID = intval($PARENT_LESSON_ID);
$ParentCourse = CLearnLesson::GetLinkedCourse ($PARENT_LESSON_ID);
if (!$ParentCourse)
    $lessonId = $PARENT_LESSON_ID;
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["Update"] <> '' && check_bitrix_sessid() && $isCreateOrEditAccess)
{
    ...
    if (!$bVarsFromForm)
    {
        ...
        if($apply == '')
        {
            ...
            {
                LocalRedirect("/bitrix/admin/learn_helper_test_admin.php?lang=".LANG
                    . "&COURSE_ID=" . $COURSE_ID
                    . '&PARENT_LESSON_ID=' . ($_GET['PARENT_LESSON_ID'] + 0)
                    . '&LESSON_PATH=' . urlencode($_GET['LESSON_PATH'])
                    .GetFilterParams("filter_", false));
        ...
        LocalRedirect("/bitrix/admin/learn_helper_test_edit.php?lang=" . LANG
            . "&COURSE_ID=" . $COURSE_ID
            . '&PARENT_LESSON_ID=' . ($_GET['PARENT_LESSON_ID'] + 0)
            . '&LESSON_PATH=' . urlencode($_GET['LESSON_PATH'])
            . "&ID=" . $ID
...
if ($lessonId){
    $str_QUESTIONS_FROM="R";
    $str_QUESTIONS_FROM_ID = $PARENT_LESSON_ID;
}
...
$aContext = array(
    array(
        ...
        "LINK"  => "learn_helper_test_admin.php?lang=" . LANG
            . '&PARENT_LESSON_ID=' . ($_GET['PARENT_LESSON_ID'] + 0)
...
if ($ID > 0)
{
    $aContext[] = 	array(
        ...
        "LINK"=>"learn_helper_test_edit.php?lang=" . LANG
            . "&COURSE_ID=" . $COURSE_ID
            . '&PARENT_LESSON_ID=' . ($_GET['PARENT_LESSON_ID'] + 0)
            . '&LESSON_PATH=' . htmlspecialcharsbx($_GET['LESSON_PATH'])
            . GetFilterParams("filter_"),
    ...
    $returnUrl = "/bitrix/admin/learn_helper_test_admin.php?lang=" . LANG
        . "&amp;COURSE_ID=" . $COURSE_ID
        . '&amp;PARENT_LESSON_ID=' . ($_GET['PARENT_LESSON_ID'] + 0)
        . '&amp;LESSON_PATH=' . urlencode($_GET['LESSON_PATH'])
        . GetFilterParams("filter_", false);
    ...
    $aContext[] = 	array(
        ...
        "LINK"	=> "javascript:if(confirm('".GetMessage("LEARNING_CONFIRM_DEL_MESSAGE")."'))window.location='learn_helper_test_admin.php?lang=".LANG
...
$tabControl->Buttons(
    array(
        ...
        "back_url" =>"learn_helper_test_admin.php?lang=". LANG
            . '&PARENT_LESSON_ID=' . ($_GET['PARENT_LESSON_ID'] + 0)
            . '&LESSON_PATH=' . htmlspecialcharsbx($_GET['LESSON_PATH'])
            . "&COURSE_ID=" . $COURSE_ID
