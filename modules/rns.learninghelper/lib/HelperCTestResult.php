...
class HelperCTestResult
{
    ...
    public function Add($arFields)
    {
        ...
        if ($this->CheckFields($arFields)) {
            ...
            $ID = $DB->Add("b_learn_test_result", $arFields, ["RESPONSE"], "learning");
    ...
    public static function AddResponse($TEST_RESULT_ID, $RESPONSE)
    {
        ...
        $rsTestResult = \CTestResult::GetList([], ["ID" => $TEST_RESULT_ID, 'CHECK_PERMISSIONS' => 'N']);
        ...
        if ($arTestResult = $rsTestResult->GetNext()) {
            ...
            }else if ($arTestResult["QUESTION_TYPE"] == "C") {  //Проверка результата ответа типа сопоставление
                $QPoint = (int)$arTestResult["QUESTION_POINT"];
                $AnswerCount = 0;
                $CorrectAnswerCount = 0;
                $pointResult = 0;
                $saveResp = "";
                $rsComparisonAnswer = ComparisonAnswer::GetList(array(), Array("QUESTION_ID" => $arTestResult["QUESTION_ID"]));
                while($arComparisonAnswer = $rsComparisonAnswer->GetNext())
                {
                    $AnswerCount++;
                    $answer = trim($arComparisonAnswer["ANSWER"]);
                    $RespAnswer = trim($arComparisonAnswer["ANSWER"]);
                    $question = trim($arComparisonAnswer["QUESTION"]);
                    $saveResp .= $question . "=". $RespAnswer. "; ";
                    if (isset($answer) && $answer == $RespAnswer)
                        $CorrectAnswerCount++;

                }
                if ($AnswerCount > 0)
                    $pointResult = ($QPoint/$AnswerCount) * $CorrectAnswerCount;
                $arFields = [
                    "ANSWERED" => "Y",
                    "RESPONSE" => $saveResp,
                    "POINT" => 0,
    ...
    public static function GetList($arOrder = [], $arFilter = [], $arNavParams = [])
    {
        ...
        $arSqlSearch = HelperCTestResult::GetFilter($arFilter);
    ...
    {
        return HelperCTestResult::GetList([], ["ID" => $ID]);
    }
