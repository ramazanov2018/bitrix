...
class LHelperTest
{
    public function CheckFields(&$arFields, $ID = false)
    {
        ...
        if ((is_set($arFields, "NAME") || $ID === false) && $arFields["NAME"] == '') {
            $arMsg[] = ["id" => "NAME", "text" => GetMessage("LEARNING_BAD_NAME")];
        }

        if ($ID === false && !is_set($arFields, "COURSE_ID"))
            $arMsg[] = ["id" => "COURSE_ID", "text" => GetMessage("LEARNING_BAD_COURSE_ID")];

        /*if (is_set($arFields, "COURSE_ID")) {
            $r = CCourse::GetByID($arFields["COURSE_ID"]);
            if (!$r->Fetch())
                $arMsg[] = ["id" => "COURSE_ID", "text" => GetMessage("LEARNING_BAD_COURSE_ID_EX")];
        }*/
        ...
        if ($arFields["APPROVED"] == "Y" &&
            ...
            $arMsg[] = ["id" => "COMPLETED_SCORE", "text" => GetMessage("LEARNING_BAD_COMPLETED_SCORE")];

        if (is_set($arFields, "PREVIOUS_TEST_ID") && intval($arFields["PREVIOUS_TEST_ID"]) != 0) {
            $r = LHelperTest::GetByID($arFields["PREVIOUS_TEST_ID"]);
            if (!$r->Fetch())
                $arMsg[] = ["id" => "PREVIOUS_TEST_ID", "text" => GetMessage("LEARNING_BAD_PREVIOUS_TEST")];
        }
        ...
        if (is_set($arFields, "PREVIOUS_TEST_SCORE") &&
            ...
            $arMsg[] = ["id" => "PREVIOUS_TEST_SCORE", "text" => GetMessage("LEARNING_BAD_COMPLETED_SCORE")];
        ...
        //Defaults
        if (is_set($arFields, "QUESTIONS_FROM") && !in_array($arFields["QUESTIONS_FROM"], ["A", "C", "L", "H", "S", 'R']))
        ...
        if (is_set($arFields, "PASSAGE_TYPE") && !in_array($arFields["PASSAGE_TYPE"], ["0", "1", "2"]))
    ...
    public function Add($arFields)
    {
        ...
        if ($this->CheckFields($arFields)) {
            ...
            $ID = $DB->Add("b_learn_test", $arFields, ["DESCRIPTION"], "learning");
    ...
    {
        return LHelperTest::GetList($arOrder = [], $arFilter = ["ID" => $ID]);
    }
    ...
    public static function GetStats($ID)
    {
        ...
        if ($arStat = $rsStat->GetNext()) {
            return ["ALL_CNT" => intval($arStat["ALL_CNT"]), "CORRECT_CNT" => intval($arStat["CORRECT_CNT"])];
        } else {
            return ["ALL_CNT" => 0, "CORRECT_CNT" => 0];
        }
    }


    public static function GetList($arOrder = [], $arFilter = [], $arNavParams = [])
    {
        global $DB, $USER;
        ...
        $oPermParser = new \CLearnParsePermissionsFromFilter ($arFilter);
        $arSqlSearch = LHelperTest::GetFilter($arFilter);
        ...
        $strSqlFrom = "FROM b_learn_test LT " . $strSqlSearch;
